###################################################################################
# This implementation has been adapted and modified from:                         #
# https://github.com/YuHengsss/VSSD/blob/main/classification/models/mamba_util.py #
...
class Mamba2(nn.Module):
    ...
    def forward(self, u, H, W, relpos, seq_idx=None, last_kv = None):
        ...
        if self.linear_attn_duality:
            ...
            assert False, "In every configuration of VMAMBA2, linear attention duality should is enabled."
...
class UperNetHead(nn.Module):
    ...
    def __init__(self, pool_scales: tuple[int, ...], in_channels: list[int], hidden_size: int, num_labels: int):
        super().__init__()
...
class VSSDSegmentation(nn.Module):
    """
    Use VSSD backbone as segmentation backbone and UperNet from transformers as decoder.
    """
    def __init__(self, num_classes: int = 1, backbone_kwargs={}, decoder_kwargs={}, **kwargs):
        super().__init__(**kwargs)
        
        self.backbone = VSSDBackbone(num_classes=num_classes, **backbone_kwargs)
        self.decoder = UperNetHead(num_labels=num_classes,**decoder_kwargs)
